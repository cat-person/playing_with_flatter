  ...
  static void logHttp({
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      try {
        int lastId =
            (OmConsole.orgLogs.isEmpty ? 0 : OmConsole.orgLogs.last.id) + 1;
        String prettyResponse = const JsonEncoder.withIndent(
          '  ',
        ).convert(response);
...
class OmConsole {
    ...
    try {
      ...
      for (Log originalLog in logs.value) {
        ...
        if (originalLog.type == LogType.http) {
          shouldSplit =
              originalLog.statusCode.toString().toLowerCase().contains(
                searchText,
              ) ||
  ...
  static void lineNavigate({bool forward = false, bool back = false}) {
  ...
  static void filterWithTags(String text) {
    try {
      ...
      if (logTypes.isEmpty || logTypes.contains(null)) {
        ...
        logs.value = orgLogs
            .where((log) => logTypes.contains(log.type))
            .toList();
      }
      if (text.isNotEmpty) {
        searchPaging();
  ...
  static void limitTotalLines({required TextStyle textStyle}) {
    ...
    for (int i = orgLogs.length - 1; i >= 0; i--) {
      ...
      if (log.type != LogType.http && log.type != LogType.sql) {
        ...
        textPainter.layout(maxWidth: maxWidth);
        final logLines = (textPainter.height / textPainter.preferredLineHeight)
            .ceil();
    ...
    for (int i = orgLogs.length - 1; i >= 0; i--) {
      ...
      if (log.type == LogType.http || log.type == LogType.sql) {
        limitedLogs.insert(
          0,
          log,
        ); // Insert HTTP logs without altering line count
      }
    }
    ...
    orgLogs.addAll(
      limitedLogs,
    ); // Ensure each log is added as an individual item
